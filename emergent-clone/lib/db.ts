...
export const db = {
  projects: {
    create: async (data: Omit<Project, 'id' | 'created_at' | 'updated_at'>): Promise<Project> => {
      ...
      const project: Project = { ...data, id, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
      projects.set(id, project)
      return project
    },
    findAll: async (): Promise<Project[]> => Array.from(projects.values()),
    findById: async (id: string): Promise<Project | null> => projects.get(id) || null,
    ...
    delete: async (id: string): Promise<boolean> => projects.delete(id),
  },
  tasks: {
    create: async (data: Omit<Task, 'id' | 'created_at'>): Promise<Task> => {
      const id = 'task_' + Date.now()
      const task: Task = { ...data, id, created_at: new Date().toISOString() }
      tasks.set(id, task)
      return task
    },
    findByProjectId: async (projectId: string): Promise<Task[]> =>
      Array.from(tasks.values()).filter(t => t.project_id === projectId),
    update: async (id: string, data: Partial<Task>): Promise<Task | null> => {
      const task = tasks.get(id)
      if (!task) return null
      const updated = { ...task, ...data }
      if (data.status === 'completed' || data.status === 'failed') updated.completed_at = new Date().toISOString()
  ...
  builds: {
    create: async (data: Omit<Build, 'id' | 'created_at'>): Promise<Build> => {
      ...
      const build: Build = { ...data, id, created_at: new Date().toISOString() }
      builds.set(id, build)
      return build
    },
    findByProjectId: async (projectId: string): Promise<Build[]> =>
      Array.from(builds.values()).filter(b => b.project_id === projectId),
    update: async (id: string, data: Partial<Build>): Promise<Build | null> => {
      const build = builds.get(id)
      if (!build) return null
      const updated = { ...build, ...data }
      if (data.status === 'success' || data.status === 'failed') updated.completed_at = new Date().toISOString()
