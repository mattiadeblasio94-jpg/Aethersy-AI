...
async function executeEmailWriter(params, ctx) {
  const { purpose, recipient, tone, context: ctxText } = params;

  const { getCompletion } = await import('./llm-provider');
  const result = await getCompletion('openrouter:qwen/qwen-2.5-72b-instruct', [
    { role: 'user', content: `Scrivi un'email ${tone} per ${recipient}. Scopo: ${purpose}. ${ctxText ? 'Contesto: ' + ctxText : ''}` }
  ]);
