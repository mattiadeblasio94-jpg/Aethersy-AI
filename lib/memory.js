...
// Wrapper sicuro per operazioni Redis (non-blocking)
async function safeRedisOp(operation, ...args) {
  try {
    const r = getRedis();
    return await operation(r, ...args);
  } catch (e) {
    console.log('[Redis] Operation skipped (DB unavailable):', e.message);
    return null;
  }
}
